...
setInterval(()=>{ // каждую секунду проверяем сеанс и обновляем текущее время
  ...
  acceptionButton.forEach(e => {
    if(selectDay.getDate() === currentDate.getDate()) {  // выключаем сеансы, которые уже прошли
      let sTime = Number(e.getAttribute("data-seance-timestamp"));// время сеанса в секундах
      let now = Math.floor(currentDate.getTime()/1000);// текущее время в секундах
       if(now < sTime){
        e.classList.remove("acceptin-button-disabled");
       } else {
        e.classList.add("acceptin-button-disabled");//удаляем класс отключенной кнопки
      }
    }
  }) 
},1000)